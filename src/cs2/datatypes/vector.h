...
struct Vector4D_t
{
	...
	Vector4D_t Conjugate() const { return Vector4D_t(-x, -y, -z, w); }
	...
	Vector_t RotateVector(const Vector_t& v) const {
		Vector4D_t vQuaternion(v.x, v.y, v.z, 0.0f);
		Vector4D_t result = *this * vQuaternion * Conjugate();
		return Vector_t(result.x, result.y, result.z);
	}
};
