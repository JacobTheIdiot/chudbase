...
struct RepeatedPtrField_t
{
	...
	T* add(T* element)
	{
		...
		static auto add_to_rep_addr = reinterpret_cast<T * (__fastcall*)(RepeatedPtrField_t*, T*)>(memory::GetAbsoluteAddress(memory::FindPattern(CLIENT_DLL, "E8 ? ? ? ? 4C 8B E0 48 8B 44 24 ? 4C 8B CF"), 0x1));
...
public:
	...
	CSubtickMoveStep* add_subtick_move()
	{
		...
		static fn_add_subtick_move_step fn_create_new_subtick_move_step = reinterpret_cast<fn_add_subtick_move_step>(memory::GetAbsoluteAddress(memory::FindPattern(CLIENT_DLL, "E8 ? ? ? ? 48 8B D0 48 8D 4F 18 E8 ? ? ? ? 48 8B D0"), 0x1));
	...
	{
		return memory::CallVFunc<int, 7U>(this);
	}
};
