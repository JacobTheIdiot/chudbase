...
public:
		...
		pMemory(nullptr), nAllocationCount(nAllocationCount), nGrowSize(nInitialGrowSize)
	{
		...
		if (nAllocationCount > 0)
			pMemory = static_cast<T*>(Interface::MemAlloc->Alloc(nAllocationCount * sizeof(T)));
	}
	...
	{
		return Interface::MemAlloc->Alloc(nSize);
	}

	void operator delete(void* pMemory)
	{
		Interface::MemAlloc->Free(pMemory);
	}
	...
	void Grow(const int nCount = 1)
	{
		...
		if (pMemory != nullptr)
			pMemory = static_cast<T*>(Interface::MemAlloc->ReAlloc(pMemory, nAllocationCount * sizeof(T)));
		else
			pMemory = static_cast<T*>(Interface::MemAlloc->Alloc(nAllocationCount * sizeof(T)));
	}
	...
	void ConvertToGrowableMemory(int nInitialGrowSize)
	{
		...
		if (nAllocationCount > 0)
		{
			...
			T* pGrowableMemory = static_cast<T*>(Interface::MemAlloc->Alloc(nByteCount));
	...
	void Purge()
	{
		...
		if (pMemory != nullptr)
		{
			Interface::MemAlloc->Free(static_cast<void*>(pMemory));
			pMemory = nullptr;
		}
