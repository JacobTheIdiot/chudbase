...
namespace console
{
	...
	struct Stream_t
	{
		...
		Stream_t& operator<<(const Vector_t& vecValue)
		{
			...
			this->iPrecision = 3;
			*this << XOR("vector3d: (") << vecValue.x << XOR(" | ") << vecValue.y << XOR(" | ") << vecValue.z << XOR(")");
			return *this;
		}
		...
		Stream_t& operator<<(const QAngle_t& angValue)
		{
			this->nModeFlags |= LOG_MODE_FLOAT_FORMAT_FIXED;
			this->iPrecision = 3;
			*this << XOR("qangle: (") << angValue.x << XOR(" | ") << angValue.y << XOR(" | ") << angValue.z << XOR(")");
			return *this;
		}

		Stream_t& operator<<(const Color_t& colValue)
		{
			*this << XOR("color: (") << static_cast<int>(colValue.r) << XOR(" | ") << static_cast<int>(colValue.g) << XOR(" | ") << static_cast<int>(colValue.b) << XOR(" | ") << static_cast<int>(colValue.a) << XOR(")");
			return *this;
		}
