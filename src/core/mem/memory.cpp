...
bool memory::Setup()
{
	...
	fnUnDecorateSymbolName = reinterpret_cast<decltype(fnUnDecorateSymbolName)>(GetExportAddress(hDbgHelp, XOR("UnDecorateSymbolName")));
	bSuccess &= (fnUnDecorateSymbolName != nullptr);

	//fnLoadKV3 = reinterpret_cast<decltype(fnLoadKV3)>(GetExportAddress(hTier0, XOR("?LoadKV3@@YA_NPEAVKeyValues3@@PEAVCUtlString@@PEBDAEBUKV3ID_t@@2@Z")));
	//bSuccess &= (fnLoadKV3 != nullptr);

	fnCreateMaterial = reinterpret_cast<decltype(fnCreateMaterial)>(FindPattern(MATERIAL_SYSTEM2_DLL, XOR("48 89 5C 24 ? 48 89 6C 24 ? 56 57 41 56 48 81 EC ? ? ? ? 48 8B 05")));
	bSuccess &= (fnCreateMaterial != nullptr);

	fnUtlBufferInit = reinterpret_cast<decltype(fnUtlBufferInit)>(GetExportAddress(hTier0, XOR("??0CUtlBuffer@@QEAA@HHW4BufferFlags_t@0@@Z")));//??0CUtlBuffer@@QEAA@HHH@Z
	bSuccess &= (fnUtlBufferInit != nullptr);

	fnUtlBufferPutString = reinterpret_cast<decltype(fnUtlBufferPutString)>(GetExportAddress(hTier0, XOR("?PutString@CUtlBuffer@@QEAAXPEBD@Z")));
	bSuccess &= (fnUtlBufferPutString != nullptr);

	fnUtlBufferEnsureCapacity = reinterpret_cast<decltype(fnUtlBufferEnsureCapacity)>(GetExportAddress(hTier0, XOR("?EnsureCapacity@CUtlBuffer@@QEAAXH@Z")));
...
void* memory::HeapAlloc(const std::size_t nSize)
{
	const HANDLE hHeap = ::GetProcessHeap();
	return ::HeapAlloc(hHeap, 0UL, nSize);
}

void memory::HeapFree(void* pMemory)
{
	if (pMemory != nullptr)
	{
		const HANDLE hHeap = ::GetProcessHeap();
...
void* memory::HeapRealloc(void* pMemory, const std::size_t nNewSize)
{
	if (pMemory == nullptr)
...
void* memory::GetModuleBaseHandle(const wchar_t* wszModuleName)
{
	...
	if (pModuleBase == nullptr)
		L_PRINT(LOG_ERROR) << XOR("module base not found: \"") << wszModuleName << XOR("\"");

	return pModuleBase;
}

const wchar_t* memory::GetModuleBaseFileName(const void* hModuleBase, const bool bGetFullPath)
{
	const _PEB* pPEB = reinterpret_cast<_PEB*>(__readgsqword(0x60));
...
void* memory::GetExportAddress(const void* hModuleBase, const char* szProcedureName)
{
	...
	{
		L_PRINT(LOG_ERROR) << XOR("module has no exports: \"") << GetModuleBaseFileName(hModuleBase) << XOR("\"");
		return nullptr;
	}
	...
	L_PRINT(LOG_ERROR) << XOR("export not found: ") << szProcedureName;
...
bool memory::GetSectionInfo(const void* hModuleBase, const char* szSectionName, std::uint8_t** ppSectionStart, std::size_t* pnSectionSize)
{
	...
	L_PRINT(LOG_ERROR) << XOR("code section not found: \"") << szSectionName << XOR("\"");
	return false;
}
...
std::uint8_t* memory::FindPattern(const wchar_t* wszModuleName, const char* szBytePattern, const char* szByteMask)
{
	...
	if (hModuleBase == nullptr)
	{
		L_PRINT(LOG_ERROR) << XOR("failed to get module handle for: \"") << wszModuleName << XOR("\"");
		return nullptr;
	}
	...
	if (pIDH->e_magic != IMAGE_DOS_SIGNATURE)
	{
		L_PRINT(LOG_ERROR) << XOR("failed to get module size, image is invalid");
		return nullptr;
	}
	...
	if (!vecFoundOccurrences.empty())
	{
		...
		if (vecFoundOccurrences.size() > 1U)
		{
			...
			L_PRINT(LOG_WARNING) << XOR("found more than one occurrence with \"") << szPattern << XOR("\" pattern, consider updating it!");
	...
	if (pFoundAddress == nullptr)
	{
		...
		L_PRINT(LOG_ERROR) << XOR("pattern not found: \"") << szPattern << XOR("\"");
